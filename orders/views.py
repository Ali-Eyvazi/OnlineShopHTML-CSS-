...
from .forms import CartAddForm, CouponAplyForm
...
class CouponApplyView(LoginRequiredMixin,View):
    ...
    def post(self,request,order_id):
        ...
        if form.is_valid():
            ...
            try:
                coupon_code=get_object_or_404(coupon,code__exact=code, valid_from__lte=now,valid_to__gte=now,active=True)
            except coupon.DoesNotExist:
                messages.error(request,'this coupon does not exists','danger')
                return redirect('orders:order_detail',order_id)
            order=Order.objects.get(id=order_id)
            order.discount=coupon_code.discount
            order.save()
            return redirect('orders:order_detail',order_id)
